...
The design is derived from the supplied BEACON visual boards: warm orange/gold signal colors, pale clinical blue, soft cream, and a restrained dark navy. It intentionally keeps the public site focused on four things: the mission, project scale, people, and ways to become involved.
...
The site uses no external image CDN and no template demo navigation. Portraits are cropped from the supplied BEACON project board. The landing page uses a CSS-built beacon/orbit visual so it stays fast, editable, and responsive without another hero asset.
